...
a zmiennymi wiek,
płeć, przyczyna zgodnu oraz dzień tygodnia (w którym nastąpił zgon) zmarłej osoby.
...
**Pomiar** -- przyporządkowanie wariantom **zmiennej** liczb lub symboli z pewnej **skali pomiarowej**.
Przykładowo jeżeli jednostką jest zgon a zmiennymi wiek, płeć przyczyna zgodnu i dzień tygodnia to
pomiar będzie polegał na ustaleniu (przyporządkowaniu) wieku w latach,
płci ('K'/'M'), przyczyny (z katalogu ICD10 zapewne)
oraz numeru dnia tygodnia (lub nazwy dnia tygodnia).
Wiek/numer są liczbami, płeć i przyczyna symbolem.
...
wypił lub nie wypił kawy. Inaczej mówić gdyby ktoś pił dużo kawy
...
W jednym z badań porównano współczynniki zgonów z miast fluoryzujących oraz
...
powoduje raka? Nie. W innym badaniu tych samych miast okazało się, że
...
ATT oznacza ATE w grupie eksperymentalnej 
...
Można do tego celu zastosować **model** regresji liniowej.
...
losowa na dwie grupy. Uczestnik badania musiał mieć minimum 60 lat oraz
wysokie ciśnienie krwi. W badaniu uczestniczyło prawie 21 tysięcy osób. 
Grupa kontrolna (przez pięć lat) używała soli zawierającej 75% chlorku potasu oraz 25%
chlorku sodu; grupa badana używała soli tradycyjnej czyli zawierającej
...
**Przykład 2: Konflikt praca-dom w zawodzie pielęgniarki (obserwacyjne)**
...
Ten konflikt jest mierzony odpowiednią skalą pomiarową składającą się z pięciu pytać.
Czynnikami które WHC mają powodować są: czasPracy, grafik (w sensie rodzaj etatu/zmianowość) 
oraz nacisk-na-nadgodziny (występuje lub nie) oraz
intensywność pracy, obciążenie emocjonalne, jakość zarządzania.
(także mierzone odpowiednimi skalami pomiarowymi). Badano 27,603 osoby.
...
z badaniem #2, w którym starano się ustalić przyczyny stanów psychicznych -:)
...
zawierają odpowiednio *Łagodne wprowadznie z Jamovi*
...
W szczególności słynne są diagramy  Nightingale, które wprawdzie
(podobno) nie okazały się szczególnie użyteczne, no ale nie każdy nowy 
...
linie pionowe (zwane wąsami) mają długość równą $Q1 - 1,5 IQR$ 
oraz $Q3 + IQR (dla przypomnienia: Q1, Q3 to kwartyle, IQR to odstęp między kwartlowy);
...
średnia jest równa `r mean.s40` a odchylenie standardowe `r  sd.s40`.
...
możemy jako metodą wykorzystać średnią-z-próby.

W ogólności metodą taką, formalnie funkcję elementów z próby, nazywa się
...
Kontynuując wnioski z eksperymetu należy zauważyć, że
wszystkie średnie-ze-średnich (bez względu na liczebność próby) są zbliżone do wartości
prawdziwej (to się nazywa **nieobciążoność**);
...
Jeżeli mamy dwa estymatory do oszacowania parametru, oba są **nieobciążone** oraz **zgodne**, to który wybrać?
...
znacznie różniące się od średniej; jeżeli jest mała to takie bardzo różniące
...
definiowany jako stosunek $\textrm{OR} = o_e/o_t$.
...
Zatem mamy rozbieżność powinno być 48/231 = 20,8% a jest 22,14% dla tych co
...
Statystk nie przyjmuje założeń na słowo honoru.
...
udostępnia dane dotyczące konsumcji żywności na świecie. Bank światowy
udostępnia dane dotyczące dochodu narodowego. 

Konsumpcja mięsa jest mierzona jako średnia konsumpacja w kilogramach w każdym kraju (*per capita* się mówi);
...
pomiędzy spożywaniem kawy w czasie sesji egzaminacyjnej a wynikiem ogzaminu
...
$Y = b_0 + b_1 \cdot X_1$ to część deterministyczna,
...
$Y = b_0 + b_1 \cdot X_1$ pomiędzy $X$ a $Y$, która w próbie
...
prostej jako zależości pomiędzy $X$ a $Y$ (prosta ale nie do końca prosta)
albo błędu pomiaru.
...
Powyższe **kryterim minimalizacyjne** oznacza,
że suma **kwadratów odchyleń wartości
empirycznych od wartości teoretycznych ma być minimalna**. Stąd **metoda największych kwadratów**
szacowania parametrów linii regresji.

Rozwiązując powyższy problem minimalizacyjny otrzymujemy wzory
definiujące parametry $b_0$ oraz $b_1$.
Przypominamy, że **estymatorem** nazywamy metodę oszacowania parametru na podstawie próby.
Ponieważ traktujemy $b_0$ oraz $b_1$ jako parametry jakieś populacj generalnej
...
Przypominamy że wartość średnia **dobrego estymatora** powinna wynosić zero (bo wtedy nie ma błądu systematycznego)
...
Graficznie **kryterim minimalizacyjne** przedstawia rysunek

![](./kmnk_roznice.png){width=75%}

Suma podniesionych do kwadratu odległości pomiędzy czerwonymi
i niebieskimi kropkami ma być minimalna. Zadanie wyznaczenie
...
dopasowana (jeżeli tylko punktów jest więcej niż dwa.) 

W oczywisty sposób regresja po lewej lepiej opisuje zależność (linia
prosta jest bliżej wartości zaobserwowanych) niż regresja po prawej.
Jak to ocenić w sposób bardziej konretny a nie tylko na oko?

**Ocena dopasowania: średni błąd szacunku**

Oznaczając *resztę* jako: $e_i = y_i - \hat y_i$, definiujemy **średni
błąd szacunku** (*mean square error*, MSE) jako:
...
oraz wariancję. Dodatkowo zakładamy, że rozkład tego estymatora jest normalny. To założenie
pozwala wiarygodnie oszacować wariancję; w konsekwencji znamy dokładny rozkład (bo przypominamy, że rozkład
...
Jeżeli takie prawdopodobieństwo jest duże, to zakładamy że być może $b_1 = 0$,
a jeżeli małe to że $b_1 \not 0$. Duże/małe przyjmujemy arbitralnie, zwykle
...
$$ \textrm{height} = \beta_0 + \beta_1 \textrm{weight}$$
...
Co znacza, że wzrost wagi o 1kg
oznacza przeciętnie większy wzrost o `r coeff_b` cm. Współczynnik determinacji
wynosi `r sprintf ("%.2f", lmr * 100)`%. 
Współczynnik nachylenia prostej jest istotny ponieważ wartość $p$
jest grubo poniżej zwyczajowego poziomu
istotności (p < 0,05). 

Kolumna `Beta` zawiera standaryzowane wartości współczynników; kolumna `CI95` zawiera
95% przedziały ufności. Z 95% prawdopodobieństwem wartość współczynnika nachylenia
...
$$\textrm{konsumpcja} = \beta_0 + \beta_1 \textrm{gdp}$$
...
W której zmienna $Y$ jest objaśniana przez wiele zmiennych $X_1, \ldots, X_k$. Wpływ każdej
...
obliczony poprzez pomnożenie współczynnika regresji $b_i$ przez $s_{xi}$
i podzielenie przez $s_y$, tj. $\beta_i = b_i s_{xi}/ s_y$. Jego interpretacja jest cokolwiek dziwacza:
zmiana zmiennej $X_i$ o jedno odchylenie standardowe (s_{xi})
skutkuje zmianą zmiennej $Y$ o $\beta_i$ jej odchylenia standardowego $s_y$.
...
Oszacowanie tego samego równania dla próba o wielkości 90 obserwacji daje następujące
wyniki:
...
Oszacowanie tego samego równania dla próba o wielkości 490 obserwacji daje następujące
wyniki:
...
$$\textrm{stres} = \ldots + b \cdot \textrm{praca}$$
...
$$\textrm{stres} = \ldots + b_4 \cdot \textrm{magister} + b_5 \cdot \textrm{licencjat} $$
...
Poprzednio rozważany model rozszerzymy o trzy zmienne: płeć (kobieta/mężczyzna),
...
Zmiennia płeć `genderF` = 1 jeżeli kobieta, lub 0 jeżeli mężczyzna. Zauważmy, że nazwa zmiennej
...
Zmienne, które okazały się istotne jednocześnie mają nawiększą wielkość efektu (kolumna `Beta`)
...
Ktoś mógły dojść do wniosku że wszystko da się **uistotnić**
...
niezbędną minimalną liczebność próby. Takie zaawanasowane podejście
wykracza poza ramy tego podręcznika.


## Regresja logistyczna

Jeżeli $Y$ jest zmienną **dwuwartościową**, to jest taką, 
która przymuje tylko dwie wartości (chory/zdrowy)
to metoda regresji nie może być zastosowana. 
Przykładowo jeżeli zakodujemy te wartości jako 0 i 1 odpowiednio,
...
Zauważmy, że $o = p/(1-p)$ to nic innego jak szansa (**odds**).
...
pewne ustalone wartości--zmienia się tylko $X_i$).

Zwykle zamiast $ln(o)$ wolimy interpretować zmianę w kategoriach
ilorazu szans: $o = exp^{\ln(o)}$. 
Jeżeli $X_i$ jest zmienną **dwuwartościową**
to interpretacja jest jeszcze prostsza: jest to iloraz szans
dla przypadku gdy $X_i=1$. 
...
Po oszacowaniu $\beta_0$ oraz $\beta_1$ możemy łatwo obliczyć $\ln(o)$.
Wiedząc że $ln(o)=p/1-p$ możemy stąd obliczyć prawdopodobieństwo, które
...
Dewiacja będzie tym większa im różnica między tymi
teoretycznymi liczebnościami a liczebnościami empirycznymi będzie większa.

Jako minimum porównuje się wielkość dewiacji szacowanego modelu
...
kobiet i mężczyzn, zatem w oczywisty sposób dewiacja tego modelu
będzie większa. Pytanie jest czy różnica jest istotna statystycznie.
Jeżeli jest przyjmuje się, że szacowany model jest lepszy od modelu
trywialnego (warunek minimum przydatności)

Jeżeli model zawiera wiele zmiennych w tym zmienne liczbowe, idea
liczenia dewiacji jest podobna, ale oczywiście szczegóły są już bardziej
...
jeżeli tak nie jest „porażkę. Wartość graniczna jest ustala albo
...
wyniosło `r prob0`. Krzywa ROC dla modelu zerowego wyląda następująco:
...
mianem refleksyjnego (co jest kalką od *reflexive*)
...
Małym utrudnieniem mogą być pytania w odwróconej skali. Jeżeli pytamy
...
To ranga 5 odpowiada uczuciu **braku strachu**. Pytania w odwróconej
skali należy przeliczyć (odwrócić): 1 zamienić na 5, 2 na 4 itd...
...
ze statusem względem palenia? Chcemy zastosować tablicę korelacyjną/test chi kwadrat. Musimy zatem zamienić skalę liczbową zmiennej  mierzącej wiedzę nt szkodliwości palenia na nominalną, np tak:
...
* Hipotezę 2--4 zweryfikowano z wykorzystaniem tablic korelacyjnych/testu chi-kwadrat
oraz porównania średniego poziomu depresji w grupach za pomocą testów Manna-Whitneya oraz
Kruskalla-Wallisa
...
Cieżką postać depresji wykazuje zaledwie 3% studentów. Należy odrzucić hipotezę
że depresja stanowi poważny problem wśród studentów RM/PO PSW.

### Weryfikacja hipotez 2--4

Aby móc zastosować metody tablicy korelacyjnej i testu chi-kwadrat
oryginalne wartości liczbowe depresji zamieniono
na skalę porządkową: 0--19 brak/łagodna depresja (B);
20--25 umiarkowana (Ł); 26--63 cieżka depresja (C).
