...
to jak widać łatwo liczyć ale jednocześnie jest to kłopotliwe. Należy do tego mieć zbiór
...
Powyższe **kryterium minimalizacyjne** oznacza,
...
Jak to ocenić w sposób bardziej konkretny a nie tylko na oko?
...
liczbowa a po prawej zmienne liczbowe lub nominalne.
...
i podzielenie przez $s_y$, tj. $\beta_i = b_i s_{xi}/ s_y$. Jego interpretacja jest cokolwiek dziwaczna:
...
która przyjmuje tylko dwie wartości (chory/zdrowy)
...
3. Wykonanie obliczeń: 
  ...
  4. Analiza zależności między zmiennymi nominalnymi: tablica korelacyjna, test
...
`$source` wpisujemy tak jak jest to zadomonstrowane (`$Source` jest błędem.)
...
### Analiza zależności: zmienna-liczbowa/zmienna liczbowa
